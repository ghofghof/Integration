...
function translateEvent(eventName) {
    ...
    $.ajax({
        ...
        data: { eventName: eventName },
        success: function(response) {
            // Mettre à jour le texte de l'événement avec la traduction
            var eventNameElement = $('[data-event-name="' + eventName + '"]');
            eventNameElement.text(response.translatedContent);
        },
        error: function(xhr, status, error) {
            console.error('Erreur lors de la traduction de l\'événement : ' + error);
        }
